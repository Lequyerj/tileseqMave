...
#analyzeLegacyTileseqCounts <- function(countfile,regionfile,outdir,logger=NULL) {
  ...
  outdir <- "/Users/Rachel/Desktop/Roth Lab/R_DMS/data/"
  logger <- NULL
	...
	# see what the net select reads are for each row (subtract the sequencing error)
	net_select1 <- rawCounts$select1 - rawCounts$controlS1
	net_select2 <- rawCounts$select2 - rawCounts$controlS2
	controls <- rawCounts[, c('controlS1', 'controlS2')]
	control_sd <- apply(controls, 1, sd)
	 
	# flag the rows if the select is likely to be zero (i.e. only due to sequencing error)
	# Flag if the net number of reads is less than or equal to 3 times the SD of the sequencing error
	flag1 <- net_select1 <= 2*control_sd
	flag2 <- net_select2 <= 2*control_sd
	
	# remove the row if it is flagged for EITHER replicate
	# i.e. if bottleneck is likely in either of the replicates
	flagged <- flag1 | flag2
	
	total <- length(rawCounts$wt_aa)
	percent <- sum(flagged)/total * 100
	
	cat("Removing ", sum(flagged), " variants (", percent, "%) due to likely bottleneck.")
  rawCounts <- rawCounts[!flagged,]
